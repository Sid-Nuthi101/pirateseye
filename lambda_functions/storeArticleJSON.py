...
def lambda_handler(event, context):
    ...
    if(edit_type == "save" and len(file["tree_elements"]) <= 0):
        return {
            'statusCode': 601,
    ...
    if edit_type == "save":
        ...
        s3_client.delete_object(Bucket=os.environ['s3BucketName'], Key='articles/drafts/article_json/'+file_name+'.json')
        
        
        # Delete from Article List
        s3_resource = boto3.resource('s3')
        content_object = s3_resource.Object(os.environ['s3BucketName'], 'articles/drafts/articles_list.json')
        file_content = content_object.get()['Body'].read().decode('utf-8')
        json_content = json.loads(file_content)
        
        del json_content[file_name]
        s3.put_object(
            Bucket=os.environ['s3BucketName'],
            Key="articles/drafts/articles_list.json",
            Body=(bytes(json.dumps(json_content).encode('UTF-8'))),
